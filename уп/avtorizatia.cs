...
namespace уп
{
    public partial class avtorizatia : Form
    {
        ...
        private void OpenUserForm(string role)
        {
            ...
            switch (role)
            {
                ...
                case "Оператор":
                    ...
                    break;
                case "Автомеханик":
                    int mechanicID = GetMechanicID();  // Получаем идентификатор автомеханика
                    requestFormMaster masterForm = new requestFormMaster(mechanicID);  // Передаем ID в конструктор
                    masterForm.Show();
                    break;
                case "Менеджер":
        ...
        private void UnblockUser(Object source, ElapsedEventArgs e)
        {
            isBlocked = false;
            attemptCount = 0;
            blockTimer.Stop();
